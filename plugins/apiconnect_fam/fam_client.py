...
class FAMAssetCatalogClient:
    ...
    def __init__(self, base_url: str, runtime_id: str, username: str, password: str, timeout: int = 30, verify_ssl: bool = True):
    ...
    async def register_runtime(
        ...
        try:
            ...
            # 200 = OK (re-registration)
            # 409 = Conflict (runtime already exists - treat as re-registration)
            status_code = response.status_code
            
            if status_code not in (200, 201, 409):
                response.raise_for_status()
            
            # Extract re-registration report from response
            response_data = response.json()
            
            # FAM API returns runtime info nested in "runtime" object
            runtime_obj = response_data.get("runtime", {})
            runtime_id_from_response = runtime_obj.get("id")
            
            if not runtime_id_from_response:
                logger.error(f"No runtime ID in response from FAM: {response_data}")
                return None
            
            # Store the runtime ID for future operations
            self.runtime_id = runtime_id_from_response
            logger.debug(f"Runtime ID from response: {runtime_id_from_response}")
            
            # Parse re-registration report (timestamps of last sync operations)
            # These fields are at the top level of the response
            report = ReregistrationReport(
                runtime_id=runtime_id_from_response,
            ...
            status_msg = {
                201: "Created (first-time registration)",
                200: "OK (re-registration)",
                409: "Conflict (runtime already exists - treated as re-registration)"
            }.get(status_code, f"Status {status_code}")
            
            logger.info(
                f"Successfully registered runtime '{name}' with ID: {runtime_id_from_response}, "
                f"status: {status_code} ({status_msg}), report: {report.model_dump()}"
            )
            return report
    ...
    async def send_heartbeat(self, runtime_id: str) -> bool:
        ...
        """
        try:
            # Build heartbeat payload with timestamp in milliseconds
            # Standard
            import time
            ...
            logger.error(f"FAM API error sending heartbeat for runtime {runtime_id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def create_server(self, server: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error creating server {server.id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def update_server(self, server: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error updating server {server.id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def delete_server(self, server_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error deleting server {server_id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def create_tool(self, tool: Any, server_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error creating tool {tool.id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def update_tool(self, tool: Any, server_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error updating tool {tool.id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def delete_tool(self, tool_id: str, server_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error deleting tool {tool_id}: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
    ...
    async def bulk_create_tools(self, tools: List[Any], server_id: str) -> Optional[str]:
        ...
        try:
            ...
            # Bulk operations return 202 Accepted with JobResponse
            if response.status_code == 202:
                job_data = response.json()
                job_id = job_data.get("jobId")
                logger.info(f"Bulk create job submitted for {len(tools)} tools " f"(server: {server_id}, job: {job_id})")
                return job_id
            else:
            ...
            logger.error(f"FAM API error bulk creating tools: " f"status={e.response.status_code}, body={e.response.text}")
            return None
        except httpx.HTTPError as e:
            logger.error(f"HTTP error bulk creating tools in FAM: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error bulk creating tools in FAM: {e}", exc_info=True)
            return None
    ...
    async def bulk_update_tools(self, tools: List[Any], server_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 202:
                job_data = response.json()
                job_id = job_data.get("jobId")
                logger.info(f"Bulk update job submitted for {len(tools)} tools " f"(server: {server_id}, job: {job_id})")
                return job_id
            else:
            ...
            logger.error(f"FAM API error bulk updating tools: " f"status={e.response.status_code}, body={e.response.text}")
            return None
        except httpx.HTTPError as e:
            logger.error(f"HTTP error bulk updating tools in FAM: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error bulk updating tools in FAM: {e}", exc_info=True)
            return None
    ...
    async def bulk_delete_tools(self, tool_ids: List[str], server_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 202:
                job_data = response.json()
                job_id = job_data.get("jobId")
                logger.info(f"Bulk delete job submitted for {len(tool_ids)} tools " f"(server: {server_id}, job: {job_id})")
                return job_id
            else:
            ...
            logger.error(f"FAM API error bulk deleting tools: " f"status={e.response.status_code}, body={e.response.text}")
            return None
        except httpx.HTTPError as e:
            logger.error(f"HTTP error bulk deleting tools from FAM: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error bulk deleting tools from FAM: {e}", exc_info=True)
            return None
    ...
    async def submit_metrics(self, metrics_payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"FAM API error submitting metrics: " f"status={e.response.status_code}, body={e.response.text}")
            return False
        except httpx.HTTPError as e:
