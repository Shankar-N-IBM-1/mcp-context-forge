...
class CheckFAMHealthActivity(AbstractScheduledActivity):
    ...
    def __init__(self, context: ActivityContext, fam_client: FAMAssetCatalogClient, check_interval: int = 30):
    ...
    async def perform(self) -> None:
        ...
        try:
            ...
            if self._total_checks == 1:
                ...
                self._average_response_time_ms = (self._average_response_time_ms * (self._total_checks - 1) + response_time_ms) / self._total_checks

            logger.info(f"FAM health check passed - response time: {response_time_ms:.2f}ms, " f"avg: {self._average_response_time_ms:.2f}ms")
            ...
            logger.error(f"FAM health check failed (consecutive failures: {self._consecutive_failures}): {e}", exc_info=True)

            # Alert if too many consecutive failures
            if self._consecutive_failures >= 3:
                logger.critical(f"FAM health check has failed {self._consecutive_failures} times consecutively! " "FAM may be unavailable.")
