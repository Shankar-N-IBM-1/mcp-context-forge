...
class SyncServersActivity(AbstractScheduledActivity):
    ...
    async def perform(self) -> None:
        ...
        try:
            ...
            servers_synced = await with_retry(
                self._query_and_sync_servers,
                retry_config=RetryConfig(max_attempts=2, initial_delay=1.0),
                operation_name="Sync Servers"
            )
            ...
            self._timestamp_handler.save_timestamp(
                self._timestamp_handler.KEY_SERVER_SYNC,
                current_time_ms
            )
            ...
            self.logger.info(
                f"Synced {servers_synced} servers to FAM (total: {self._total_servers_synced})"
            )
    ...
    def get_sync_stats(self) -> dict:
        ...
        """
        return {
            "total_synced": self._total_servers_synced,
