...
from .handlers.timestamp_handler import TimestampStorageHandler
...
class ActivityOrchestrator:
    ...
    def __init__(
        ...
        Args:
            ...
            metrics_interval: Metrics sync interval in seconds (default: 300)
            server_sync_interval: Server sync interval in seconds (default: 60)
            tool_sync_interval: Tool sync interval in seconds (default: 60)
        ...
        # Initialize handlers
        self.timestamp_handler = TimestampStorageHandler(
            storage_path=f"data/agent_timestamps_{runtime_id}.json"
        )
        ...
        self.fam_health_activity = CheckFAMHealthActivity(
            context=self.context,
            fam_client=fam_client,
            check_interval=fam_health_check_interval
        )
        self.activities.append(self.fam_health_activity)
        
        self.runtime_health_activity = CheckRuntimeHealthActivity(
            context=self.context,
            check_interval=runtime_health_check_interval
        )
        self.activities.append(self.runtime_health_activity)
        
        # Create heartbeat activity (independent)
        self.heartbeat_activity = SendHeartbeatActivity(
            context=self.context,
            fam_client=fam_client,
            timestamp_handler=self.timestamp_handler,
            heartbeat_interval=heartbeat_interval
        )
        self.activities.append(self.heartbeat_activity)
        
        # Create metrics activity (independent)
        self.metrics_activity = SendMetricsActivity(
            context=self.context,
            fam_client=fam_client,
            timestamp_handler=self.timestamp_handler,
        ...
        # Create server sync activity (must run before tools)
        self.server_sync_activity = SyncServersActivity(
            context=self.context,
            fam_client=fam_client,
            timestamp_handler=self.timestamp_handler,
            sync_interval=server_sync_interval,
            orchestrator=self  # Pass self for server ID mapping
        )
        self.activities.append(self.server_sync_activity)
        
        # Create tool sync activity (depends on servers)
        self.tool_sync_activity = SyncToolsActivity(
            context=self.context,
            fam_client=fam_client,
            timestamp_handler=self.timestamp_handler,
            sync_interval=tool_sync_interval,
            orchestrator=self  # Pass self to get server ID mapping
        )
        ...
        logger.info(
            ...
            f"heartbeat={heartbeat_interval}s, metrics={metrics_interval}s, "
            f"servers={server_sync_interval}s, tools={tool_sync_interval}s"
        )
                    ...
                    if activity.should_execute():
                        ...
                        if activity == self.tool_sync_activity:
                            if not self._servers_synced_this_cycle:
                                logger.debug(
                                    "Skipping tool sync - waiting for server sync to complete first"
                                )
                                continue
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error executing activity {activity.__class__.__name__}: {e}",
                                exc_info=True
                            )
    ...
    def get_statistics(self) -> SyncStatistics:
        ...
        # Get specific counts from activities
        stats.total_heartbeats_sent = self.heartbeat_activity.get_heartbeat_stats().get("total_sent", 0)
        stats.total_metrics_sent = self.metrics_activity.get_metrics_stats().get("total_sent", 0)
        stats.total_servers_synced = self.server_sync_activity.get_sync_stats().get("total_synced", 0)
        stats.total_tools_synced = self.tool_sync_activity.get_sync_stats().get("total_synced", 0)
    ...
    def get_activity_statistics(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            activity.__class__.__name__: activity.get_statistics().model_dump()
            for activity in self.activities
        }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_checks": {
                "fam": self.fam_health_activity.get_health_stats(),
                "runtime": self.runtime_health_activity.get_health_stats()
            },
            "heartbeat_status": self.heartbeat_activity.get_heartbeat_stats(),
            "metrics_status": self.metrics_activity.get_metrics_stats()
        }
